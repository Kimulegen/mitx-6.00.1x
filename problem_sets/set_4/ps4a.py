...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    score = sum(map(lambda x: SCRABBLE_LETTER_VALUES[x], word)) * len(word)
...
def playGame(wordList):
    ...
    hands_played = []
    while user_choice != "e":
        if user_choice == "r" and len(hands_played) == 0:
            print("You have not played a hand yet. Please play a new hand first!\n")

        elif user_choice == "n":
            hands_played.append(dealHand(HAND_SIZE))
            playHand(hands_played[-1], wordList, HAND_SIZE)
